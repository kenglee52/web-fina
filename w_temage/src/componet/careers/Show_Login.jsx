...
const Show_Login = () => {
  ...
  const MobileLayout = () => (
    <div className="max-w-4xl w-full text-center">
            ...
            >
              ...
              <div className="text-left flex-1">
                ...
                <p className="text-gray-600 text-xs">
                  ...
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${post.status === 'open'
                ...
                <button
                  className={`mt-2 px-2 py-1 rounded-lg text-white font-semibold text-xs transition-all duration-300 ${isExpired(post.end_date)
      ...
      <SignInButton mode="modal">
        <button className="bg-orange-500 text-white px-6 py-3 rounded-lg hover:bg-orange-600 transition-all duration-300 font-semibold">
          Sign In
        </button>
      </SignInButton>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white rounded-3xl w-full p-4 shadow-2xl relative transform scale-100 transition-all duration-300 overflow-y-auto max-h-[90vh]">
            ...
            <div className="pt-6 border-t border-gray-200">
              <SignInButton mode="modal">
                <button
                  className={`flex items-center justify-center gap-2 text-white px-4 py-3 rounded-xl shadow-lg transition-all duration-300 font-bold text-base w-full ${isPostClosedOrExpired(selectedPost)
                      ? 'bg-gray-400 cursor-not-allowed'
                      : 'bg-gradient-to-r from-orange-500 to-red-500 hover:shadow-xl hover:from-orange-600 hover:to-red-600'
                    }`}
                  disabled={isPostClosedOrExpired(selectedPost)}
                >
                  {isPostClosedOrExpired(selectedPost) ? (
          ...
          {jobPosts.map((post, index) => (
            ...
            >
              ...
              <div className="text-left flex-1">
                ...
                <button
                  className={`mt-2 px-2 py-1 rounded-lg text-white font-semibold text-sm transition-all duration-300 ${isExpired(post.end_date)
                      ? 'bg-gray-400 cursor-not-allowed'
                      : 'bg-orange-500 hover:bg-orange-600'
                    }`}
                  disabled={isExpired(post.end_date)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="bg-white rounded-3xl max-w-lg w-full p-8 shadow-2xl relative transform scale-100 transition-all duration-300 overflow-y-auto max-h-[90vh]">
            ...
            <div className="pt-6 border-t border-gray-200">
              <SignInButton mode="modal">
                <button
                  className={`flex items-center justify-center gap-2 text-white px-8 py-4 rounded-xl shadow-lg transition-all duration-300 font-bold text-lg w-full ${isPostClosedOrExpired(selectedPost)
                      ? 'bg-gray-400 cursor-not-allowed'
                      : 'bg-gradient-to-r from-orange-500 to-red-500 hover:shadow-xl hover:from-orange-600 hover:to-red-600'
                    }`}
                  disabled={isPostClosedOrExpired(selectedPost)}
  ...
  return (
    <div className="min-h-[500px] flex flex-col items-center justify-center p-6 bg-white">
      <SignedOut>{isMobile ? <MobileLayout /> : <DesktopLayout />}</SignedOut>
    </div>
  );
};
