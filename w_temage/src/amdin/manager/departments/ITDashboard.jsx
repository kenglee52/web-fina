...
const ITDashboard = () => {
  ...
  // Redirect if not an IT manager or no admin data
  if (!admin || admin.department_name !== 'IT') {
  ...
  useEffect(() => {
    const fetchApplicants = async () => {
      ...
      try {
        ...
        // Fetch applicants for IT department
        console.log('Fetching IT applicants with token:', token);
        const applicantsResponse = await axios.get(`${url.base_url}/api/applicants_manager?department=IT`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        console.log('API Response (Applicants):', applicantsResponse.data);
        setApplicants(applicantsResponse.data.applicants || []);

        // Fetch unread count for IT department
        const unreadResponse = await axios.put(`${url.base_url}/api/applicants/unread?department=IT`, {}, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleStatusUpdate = async () => {
    ...
    try {
      ...
      setUpdateStatus({
        type: 'error',
        message: err.response?.data?.error || `Pls Login again. Failed to update status to ${selectedStatus} `,
      });
    } finally {
    ...
    <div className="max-w-full mx-auto p-6 bg-gradient-to-br from-white to-white min-h-screen">
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-4xl font-bold">
          ...
          <span className="text-sky-900">Department Dashboard</span>
        </h1>
        <button
          onClick={handleLogout}
          className="flex items-center gap-2 px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-all duration-300 font-semibold"
          aria-label="Logout"
        >
          <LogOut size={20} />
          Logout
      ...
      <div className="mb-6 flex items-center justify-center">
        <div className="relative flex items-center gap-2 px-4 py-2 bg-orange-100 text-orange-900 rounded-xl shadow-md hover:bg-orange-200 transition-all duration-300">
      ...
      {error && (
        <div className="p-4 mb-6 rounded-xl text-center bg-red-100 text-red-700">
          {error}
        </div>
      )}
      {updateStatus && (
        <div
          className={`p-4 mb-6 rounded-xl text-center ${updateStatus.type === 'success'
      ...
      {loading ? (
        ...
        <div className="text-center text-gray-600">No applicants found for IT department.</div>
      ) : (
              ...
              {applicants.map((applicant) => {
                ...
                return (
                  <React.Fragment key={applicant.applicant_id}>
                    <tr
                      className={`border-b border-gray-200 hover:bg-gray-50 ${!applicant.manager_read ? 'bg-orange-100 border-l-4 border-orange-500' : ''
                        }`}
                    >
            ...
            <div className="flex min-h-full items-center justify-center p-4">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  <Dialog.Title
                    as="h3"
                    className="text-lg font-semibold leading-6 text-sky-900"
                  >
