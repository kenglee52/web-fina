...
const Reports = () => {
  ddEventListener("mouseenter", Swal.stopTimer);
      toast.addEvconst [sidebarOpen, setSidebarOpen] = useState(false);
  ...
  const Toast = Swal.mixin({
    ...
    didOpen: (toast) => {
      toast.aentListener("mouseleave", Swal.resumeTimer);
    },
  });
  ...
  return (
    <div className="flex min-h-screen bg-green-50">
      {/* Sidebar */}
   <div
  className={`bg-gradient-to-b from-green-900 to-green-700 text-white min-w-max sm:max-w-96 flex flex-col transition-all duration-300 shadow-xl ${
...
>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-4 px-5 py-3 rounded-lg transition-all font-noto-sans-lao ${
                isActive ? "bg-green-600 text-white text-3xl" : "hover:bg-green-600 hover:text-white text-2xl"
              }`
            }
        ...
        <div className="p-4 sm:p-6 space-y-3">
          <button
            className="w-full flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-lg shadow-md transition-all font-noto-sans-lao text-2xl"
      ...
      <div className="flex-1 flex flex-col">
        ...
        <header className="bg-white h-20 shadow-lg flex items-center justify-between px-4 sm:px-8 text-xl font-semibold border-b-2 border-green-600 rounded-b-xl font-noto-sans-lao">
          ...
          <span className="text-2xl">ແຜງໜ້າປັດລາຍງານ</span>
        </header>
